...
def rospkg_exists(name):
    ...
    try:
        ...
        get_middleware().logwarn('package {name} not found'.format(name=name))
        return False
    if len(version_entry1) == 1:
        return True
    if not compare_version(version_entry1[2], version_entry1[1], m.version):
        get_middleware().logwarn('found ROS package {installed_name}=={installed_version} but {r} is required}'.format(
...
def check_dependencies():
    ...
    for d in dependencies:
        try:
            ...
            get_middleware().logwarn(f'found {str(e.dist)} but version {str(e.req)} is required')
