...
def grow_tree():
    ...
    sync.add_plugin(u'js', success_is_running(ConfigurationPlugin)(u'js', map_frame))
    sync.add_plugin(u'pybullet updater', WorldUpdatePlugin(u'pybullet updater'))
    # sync.add_plugin(u'knowrob sync', KnowrobPlugin())
    sync.add_plugin(u'in sync', SuccessPlugin(u'in sync'))
    ...
    actual_planning.add_plugin(u'kin sim', KinSimPlugin(u'kin sim', sample_period))
    actual_planning.add_plugin(u'coll', CollisionChecker(u'coll', default_collision_avoidance_distance, map_frame, root_link, marker))
    actual_planning.add_plugin(u'controller', ControllerPlugin(u'controller', path_to_data_folder, nWSR))
    actual_planning.add_plugin(u'log', LogTrajPlugin(u'log'))
    actual_planning.add_plugin(u'goal reached', GoalReachedPlugin(u'goal reached', joint_convergence_threshold))
    actual_planning.add_plugin(u'wiggle', WiggleCancel(u'wiggle', wiggle_precision_threshold))
