...
if __name__ == '__main__':
    ...
    try:
        ...
        if result.error_codes == result.SUCCESS:
            rospy.loginfo('cone \'{}\' added'.format(name))
        else:
            rospy.logwarn('failed to add cone \'{}\''.format(name))
    except KeyError:
        rospy.loginfo(
            'Example call: rosrun giskardpy add_cone.py _name:=cone _size:=[1,1] _frame_id:=map _position:=[0,0,0] _orientation:=[0,0,0,1]')
