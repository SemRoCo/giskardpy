        ...
        if self.parent.monitor.get_state():
            for monitor in GiskardBlackboard().tree.control_loop_branch.check_monitors.children:
                if monitor == self.parent or (hasattr(monitor, 'original') and monitor.original == self.parent):
                    ...
                    current_life_cycle_state = god_map.motion_graph_manager.monitor_life_cycle_state[monitor_id]
                    if current_life_cycle_state not in [LifeCycleState.succeeded, LifeCycleState.failed]:
                        god_map.motion_graph_manager.monitor_life_cycle_state[monitor_id] = LifeCycleState.succeeded
