...
import numpy as np
from mujoco_msgs.msg import ObjectStateArray, ObjectState
...
class MujocoBBDetector:
    def __init__(self, sub_topic: str, pub_topic: str):
        ...
        rospy.Subscriber('/mujoco/object_states', ObjectStateArray, self.callback2)
    ...
    def callback2(self, data: ObjectStateArray):
        for state in data.object_states:
            assert isinstance(state, ObjectState)
            if 'ball' not in state.name:
                self.object_states[state.name] = state.pose
